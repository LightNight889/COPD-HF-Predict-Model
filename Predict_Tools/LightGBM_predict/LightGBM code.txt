...
class COPDReadmissionPredictor:
    def __init__(self, root):
        self.root = root
        self.root.title("COPD患者6个月再入院风险预测系统 (LightGBM)")
        ...
        if self.model is None:
            messagebox.showerror("错误", "无法加载模型文件，请确保lightgbm_model.pkl与程序在同一目录")
            return
    ...
    def check_current_directory(self):
        ...
        print(f"模型文件路径: {os.path.join(current_dir, 'lightgbm_model.pkl')}")
        print(f"模型文件是否存在: {os.path.exists(os.path.join(current_dir, 'lightgbm_model.pkl'))}")
    ...
    def load_model(self):
        ...
        try:
            ...
            if not os.path.exists(model_path):
                ...
                # 尝试在工作目录查找
                work_dir_model = "lightgbm_model.pkl"
    ...
    def predict(self):
        ...
        # 显示结果
        result_text = f"此COPD合并病人6个月再入院的概率为{probability_percent}% (LightGBM模型)"
        ...
        try:
            ...
            if SHAP_AVAILABLE and self.explainer is not None:
                try:
                    ...
                    self.ax.set_title('基于SHAP的特征重要性分析 (LightGBM)')
    ...
    def fallback_feature_importance(self, display_feature_names):
        ...
        try:
            ...
            if hasattr(self.model, 'feature_importances_'):
                ...
                self.ax.set_title('基于LightGBM的特征重要性分析')
            else:
